...
import { TroCapItem } from "@/dtos/danhMuc/tro-cap/troCap.dto";
...
import DanhMucTroCapServices from "@/services/danh-muc/tro-cap/troCap.service";
...
function Page() {
  ...
  const [rowData, setRowData] = useState<TroCapItem[]>([]);
  ...
  const columnDefs: ColDef[] = useMemo(
    () => [
      {
        field: "allowanceCode",
        headerName: "Mã trợ cấp",
        editable: true,
        width: 150,
        cellStyle: (params) => {
          const itemId = params.data ? getItemId(params.data) : "";
          return itemErrorCellStyle(itemId, "allowanceCode", params);
        },
      },
      {
        field: "allowanceName",
        headerName: "Tên trợ cấp",
        editable: true,
        width: 200,
      },
      {
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        const response = await DanhMucTroCapServices.getDanhMucTroCap(
          searchFilter,
          quickSearchText
        );
  ...
  const dataGrid = useDataGridOperations<TroCapItem>({
    gridRef,
    createNewItem: (i) => ({
    ...
    setRowData,
    requiredFields: [
      { field: "allowanceCode", label: "Mã trợ cấp" },
      { field: "allowanceName", label: "Tên trợ cấp" },
      { field: "allowanceAmount", label: "Số tiền trợ cấp" },
    ],
    t,
  ...
  const handleSave = dataGrid.createSaveHandler(
    DanhMucTroCapServices.createDanhMucTroCap,
    DanhMucTroCapServices.updateDanhMucTroCap,
  ...
  const handleDelete = dataGrid.createDeleteHandler(
    DanhMucTroCapServices.deleteDanhMucTroCap,
    () => fetchData(currentPage, pageSize, quickSearchText)
  );
